#!/usr/bin/env python3
"""
hypr_dotfiles installer for NixOS
...
def install_packages():
    header("Paketler Yükleniyor (NixOS)")

    if not is_nixos():
        warn("Bu installer NixOS için tasarlandı, ama devam ediliyor.")

    # Zaten yüklü olanları tespit et
    already, missing = [], []
    for pkg in NIX_PACKAGES:
        result = run(f"nix-env -q {pkg}", check=False, capture=True)
    ...
    try:
        run(f"nix-env -iA nixpkgs.{' '.join(missing)}")
        ok("Paketler başarıyla yüklendi.")
    except subprocess.CalledProcessError:
        error("Paket kurulumu başarısız. Paket adlarını ve internet bağlantını kontrol et.")
        sys.exit(1)
...
def print_summary():
  ...
  4. nvibrant (NVIDIA renk aracı) istiyorsan:
       {CYAN}nix-env -iA nixpkgs.nvibrant{RESET}
""")
